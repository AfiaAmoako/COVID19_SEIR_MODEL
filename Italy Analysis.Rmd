...
Callibrating the high transmission based on current information (priors) [in a perfect world, i would construct a likelihood function and combine with prior to create a model but it is what it is] chose to callibrate for 40 days so basically when cases in Italy were very strong and government were taking stronger initiative
```{r}
...
R0_1 <- 1.6 
R0_2 <- 2.6
R0_3 <- 3.6 #without any intervention
R0_4 <- 0.9
...
#"Estimated Number of People Infectious in \n  Italy with varying R0 values",
par(mfrow = c(1, 2))
plot(high_beta_italy$I ~ axis_italy, type = 'l',  xlab = "Date", ylab = "Number of People Infectious", col = 'navy', lwd = 3, main = "A")
 lines(medium_beta_italy$I ~ axis_italy, col = 'red', lwd = 3)
 lines(low_beta_italy$I ~ axis_italy, col = 'pink', lwd = 3)
 legend("topleft", legend = c( "Model R: 3.5","Model R:2.6", "Model R: 1.5"), fill = c("navy", "red", "pink"), col = c("navy", "red", "pink"), cex = 0.75, box.lty = 0)

 
#"Comparing Modeled Cumulative Cases in Italy with varying R0 values to current cases",
plot(cum.cases.italy[c(10:50), 1], cum.cases.italy[c(10:50), 2], type = 'l', xlab = "Date", ylab = "Cumulative Cases", col = "chartreuse4", lwd = 3, main = "B")
 ...
 legend("topleft", legend = c("Current Cases \n in Italy", "Model R: 3.5","Model R:2.6", "Model R:1.5"), fill = c("chartreuse4", "navy", "red", "pink"), col = c("chartreuse4", "navy", "red", "pink"), cex = 0.75, box.lty = 0)
...
#after graphing, it seem that 3.5 is actually a better growth rate because it is similar to the italy number assuming high transmission rate. Assuming that italy is only counting those who are presenting for testing, tends to be those with some symptoms and this is not including those who are incubating, so could be under estimation. Thus used the I to represent the symptomatic and calculated cumulative cases by the number of cases moving from E to I each time step and then taking the  cumulative sum. 3.5 is a good R0 to start with. 
```
...
plot(X_italy_p[c(1:24), 3] ~ axis_italy_p[c(1:24)] ,  col = 'steelblue4',  lwd = 3,  type = 'l', ylab = "Number of People Infectious", xlab = "Time (Days)",  xlim = as.Date(c("2020-01-31", "2020-04-30")), ylim = c(0,20000), main = "A")
  ...
  lines(X_italy_p_n[c(1:91), 3] ~ axis_italy_p[c(1:91)] , col = 'navy', lwd = 3) #if we are assuming that transmission continued at a high rate
legend("topleft", legend = c("R=3.5", "R=2.6", "R=1.5", "R=0.5", "No Intervention"), fill = c("steelblue4", "violetred4", "gold", "red", "navy"), col = c("steelblue4", "violetred4", "gold", "red", "navy"), cex = 0.5, box.lty = 0)

#cumulative cases comparison
#without intervention
plot(cumsum(X_italy_p_n[c(1:91), 5]) ~ axis_italy_p[c(1:91)],  col = 'navy',  lwd = 3,  type = 'l', ylab = 'Cumulative Cases', xlab = "Time (Days)",  xlim = as.Date(c("2020-01-31", "2020-04-30")), ylim = c(0,50000), main = "B") 
#current situation
lines(cum.cases.italy[c(10:73), 2] ~ axis_italy_p[c(1:64)], col = 'chartreuse4', lwd = 3) 
#with intervetnion
 lines(cumsum(X_italy_p[c(1:91), 5]) ~ axis_italy_p[c(1:91)],  col = 'red', lwd = 3) 
legend("topleft", legend = c("Without Interventions", "Current Situation", "With Interventions"), fill = c("navy", "chartreuse4", "red"), col = c("navy", "chartreuse4", "red"), cex = 0.5, box.lty = 0)
...
par(mfrow = c(1, 2))
#plot: number of new cases main = "Impact of Intervention on the Number of Infectious People Each Day",
plot(X_italy_p_e[c(1:18), 3] ~ axis_italy_p[c(1:18)] , type = 'l', col = 'steelblue4',  lwd = 3,  ylab = "Number of People Infectious", xlab = "Time (Days)",  xlim = as.Date(c("2020-01-31", "2020-04-30")), ylim = c(0,20000), main = "A")
...
legend("topright", legend = c("R=3.5", "R=2.6", "R=1.5", "R=0.5", "No Intervention"), fill = c("steelblue4", "violetred4", "gold", "red", "navy"), col = c("steelblue4", "violetred4", "gold", "red", "navy"), cex = 0.5, box.lty = 0)



#cumulative cases comparison
#intervention: main = "Comparing the Model Interventions with Non-intervention",
plot(cumsum(X_italy_p_e[c(1:91), 5]) ~ axis_italy_p[c(1:91)] , type = 'l', col = 'pink',  lwd = 3,  ylab = 'Cumulative Cases', xlab = "Time (Days)",  xlim = as.Date(c("2020-01-31", "2020-04-30")), ylim = c(0,50000), main = "B") #with intervetnion
 lines(cumsum(X_italy_p_n[c(1:91), 5]) ~ axis_italy_p[c(1:91)] , col = 'navy', lwd = 3) 
lines(cum.cases.italy[c(10:73), 2] ~ axis_italy_p[c(1:64)], col = 'chartreuse4', lwd = 3) 
lines(cumsum(X_italy_p[c(1:91), 5]) ~ axis_italy_p[c(1:91)] ,  col = 'red',  lwd = 3)
legend("topright", legend = c("Without Interventions", "Current Situation", "With Interventions", "Intervention A \n week earlier"), fill = c("navy", "chartreuse4", "red","pink" ), col = c("navy", "chartreuse4", "red","pink" ), cex = 0.4, box.lty = 0)
...
#Comparing Values if the intervention was earlier
```{r}
